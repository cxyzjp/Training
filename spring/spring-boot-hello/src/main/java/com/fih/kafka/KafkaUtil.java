...
public class KafkaUtil {
	...
	static {
		...
		// kafka服务器地址
		config.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "192.168.206.100:9092,192.168.206.100:9093,192.168.206.100:9094");
		...
		// zookeeper地址
		props.put("zookeeper.connect", "192.168.206.100:2181");
		// 组id
	...
	public static void main(String[] args) throws Exception {
		//发送一个信息
		send("test",new KafkaBean(1001, "userName"));
		//为test启动一个消费者，启动后每次有消息则打印对象信息
		KafkaUtil.startConsumer("test", new MqMessageHandler<KafkaBean>() {
			@Override
			public void handle(KafkaBean bean) {
				//实现自己的处理逻辑，这里只打印出消息
